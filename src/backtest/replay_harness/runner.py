...
class BacktestRunner:
    ...
    def __init__(
        ...
        *,
        ...
        # 1m bars improve fill precision but are optional; the exchange sim
        # falls back to coarser timeframes (15m, 1h, …) when 1m is absent.
        # Deep 1m history is expensive to obtain, so allow callers to omit it.
    ...
    async def _initialize(self) -> None:
        ...
        if hasattr(lt, "candle_manager"):
            ...
            lt.candle_manager._now_fn = self._clock.now
            # Skip DB lookups — production DB timestamps poison the
            # `since` parameter and filter out all replay-era candles.
            lt.candle_manager._skip_db_lookups = True
            # Re-wire OHLCVFetcher client — it takes priority over
